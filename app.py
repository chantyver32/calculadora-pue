...
if btn_calcular:
    if not opcion:
        st.warning("⚠️ Por favor, selecciona un artículo.")
    elif peso_total is None:
        st.warning("⚠️ Ingresa el peso registrado en la báscula.")
    else:
        pue = productos[opcion]
        ...
        if peso_neto < 0:
            ...
            st.info("ℹ️ Este artículo se registra por unidad directa (Kilo/Litro/Pza).")
        elif pue == 0:
            st.warning("⚠️ Este artículo no tiene un factor PUE definido.")
        else:
            # Cálculo de resultado
            # Para tinta usamos la fórmula específica, para otros el peso_neto / pue
            if es_tinta:
                resultado = (peso_total - 0.030) / 0.078
                txt_formula = f"({peso_total:.3f} - 0.030) / 0.078"
            else:
                resultado = peso_neto / pue
                txt_formula = f"({peso_total:.3f} - {peso_tara:.4f}) / {pue}" if usar_tara else f"{peso_total:.3f} / {pue}"

            # Mostrar resultado
            st.divider()
            st.metric(label=f"Cantidad estimada para {opcion}", value=f"{resultado:.2f}")
            
            with st.expander("Ver detalles del cálculo"):
                st.write(f"**Fórmula:** {txt_formula}")
                st.write(f"**PUE:** {pue}")
                st.write(f"**Peso Neto:** {peso_neto:.4f} kg")
